			...
			<view v-show="classify === '行为隐患'">
				<view>
					...
					<view class="qiun-bg-white qiun-title-bar qiun-common-mt" style="display: flex;justify-content: center;align-items: center;">
						<view class="qiun-title-dot-light">行为隐患数趋势</view>
					</view>
				...
				<view class="uni-list-cell"  v-for="(oldAgent,index) in selectAgent" :key="index">
	...
	export default {
		...
		methods: 
		{
			...
			getAppOldAgent() {
				...
				var urlbuf = ''
				if(this.classify === '行为隐患')
				{
					urlbuf = 'Danger/GetHisActionDangers';
				}
				else if(this.classify === '设备隐患')
				{
					urlbuf = 'Danger/GetHisDeviceDangers'
				}
				uni.showToast({
				    title: '加载中...',
					icon:'loading',
				...
				uni.request({
					...
					success: (res) => {
						...
						}else
						{
							...
							{
								this.selectAgent.push(res.data.Data[i])
							}
							if(res.data.Data.length < 10)
							{
								this.loadMoreText = "没有更多数据了!"
							}
							this.selectAgentlength = this.selectAgent.length
